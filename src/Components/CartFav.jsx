...
const DesktopCartMenu = ({ cartCount, RemoveAll, removeItem, isOpen, setIsOpen }) => {
  ...
  return (
    <div
      className={`hidden md:block origin-top-right absolute -right-4 top-7 mt-3 w-80 rounded-xl bg-white shadow-2xl ring-1 ring-black ring-opacity-5 z-50 transition-opacity duration-300 ${isOpen ? "opacity-100 scale-100" : "opacity-0 scale-95 pointer-events-none"
        }`}
      role="menu"
        ...
        <div className="max-h-80 overflow-y-auto divide-y divide-gray-100 mt-2">
          {cartCount.length ? (
            ...
            <div className="text-gray-500 flex flex-col justify-center items-center py-8">
              <Heart size={36} className="mb-3 text-gray-300" />
        ...
        <div className="flex-1 overflow-y-auto divide-y divide-gray-100 p-4">
          {cartCount.length ? (
            ...
            <div className="flex flex-col justify-center items-center mt-10 text-gray-500">
              <Heart size={50} className="mb-3 text-gray-300" />
